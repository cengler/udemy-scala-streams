...
object C14GraphCycles extends App {
  ...
  val fibonacciGraph = GraphDSL.create() { implicit builder =>
    ...
    val n = builder.add(Source(List(1)))
    val n1 = builder.add(Source(List(1)))
    val mergeN = builder.add(MergePreferred[Int](1))
    val mergeN1 = builder.add(MergePreferred[Int](1))
    val b = builder.add(Broadcast[Int](2))
    val zip = builder.add(ZipWith[Int, Int, Int](_ + _))
    val print = builder.add(Sink.foreach[Int](e => {
      Thread.sleep(1000)
      println(e)
    }))

    val b2 = builder.add(Broadcast[Int](2))

    n ~> mergeN ~> zip.in0;
    zip.out ~> b2 ~> mergeN1.preferred
    n1 ~> mergeN1 ~> b ~> mergeN.preferred
    b ~> zip.in1
    b2 ~> print
